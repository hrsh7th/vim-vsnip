...
function! vsnip#definition#invalidate(filepath) abort
  if has_key(s:cache, a:filepath)
    unlet s:cache[a:filepath]
  endif
endfunction

function! vsnip#definition#get_prefixes(filetype) abort
  let l:prefixes = []
...
function! vsnip#definition#get_filepaths(filetype) abort
  let l:filepaths = []
...
function! vsnip#definition#get_snippets(filetype) abort
  let l:snippets = []
  for l:filepath in vsnip#definition#get_filepaths(a:filetype)
...
function! vsnip#definition#get_snippet_with_prefix_under_cursor(filetype) abort
  let l:snippets = vsnip#definition#get_snippets(a:filetype)
  if empty(l:snippets)
    return {}
  endif
